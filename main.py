...
def get_word():  # Функци выбора рандомного слова из списка
...
def hide_letters(word):  # Функция шифрования слова
...
def reveal_letter(guess, word=word, hidden_word=hidden_word):  # Открывает букву в скрытом слове, если она есть.
...
def call_ans():  # Главный алгоритм игры
    print(f"\033[32mЯ загадал слово (сущ.), у тебя есть 6 попыток, чтобы отгадать это слово. Поехали....\033[0m")  # Зеленый цвет
    time.sleep(2)
    print(f"\033[32mЗагаданное слово: \033[0m\033[33m{' '.join(hidden_word)}\033[0m")
    death = 0
    while death < 6:
        guess = input("\033[32mВведите букву: \033[0m")  # Зеленый цвет
        reveal_letter(guess)
        print("\033[32m" + " ".join(hidden_word) + "\033[0m")  # Зеленый цвет
        if guess not in word:  # Проверяем, есть ли буква в слове
            death += 1  # Буквы нет, счетчик +1
            print(f"\033[32mНеверный ответ! Количество ошибок {death}\033[0m")  # Зеленый цвет
            if death == 6:
                print("\033[31mВы проиграли...\033[0m")  # Красный цвет
                time.sleep(2)
...
def show_intro_message():
    print(
        "\033[32m"  # Установка зеленого цвета
        "Привет, мой друг.\n"
